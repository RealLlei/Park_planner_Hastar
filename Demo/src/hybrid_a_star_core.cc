...
#include "hybrid_a_star.h"  // 混合A*算法的头文件

// 构造函数，初始化混合A*算法的配置和车辆参数
HybridAStar::HybridAStar(const WarmStartConfig& warm_start_config,
    ...
    : warm_start_config_(warm_start_config), vehicle_param_(vehicle_param) {
      // ：代表 列表初始化，使用列表初始化方式，因为是const or 引用传递

  /* 初始化网格搜索启发式生成器。
  创建了一个能调用GridSearch类功能的智能指针管理器grid_a_star_heuristic_generator_，
  grid_a_star_heuristic_generator_的初始值为warm_start_config_
  通过这个智能指针可以访问和操作实际的GridSearch对象*/
