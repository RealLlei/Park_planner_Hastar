...
// 该文件定义了 Node3d 类，表示三维搜索空间中的节点。
// 节点用于混合 A* 算法，存储位置、方向和代价信息。

// Node3d:
// - x_: 节点的 x 坐标。
// - y_: 节点的 y 坐标。
// - phi_: 节点的方向角。
// - traj_cost_: 从起点到该节点的轨迹代价。
// - heuristic_cost_: 从该节点到目标的启发式代价。
// - cost_: 总代价，包括轨迹代价和启发式代价。
// - pre_node_: 指向前驱节点的指针。
// - traversed_x_, traversed_y_, traversed_phi_: 记录从前驱节点到当前节点的轨迹。
// 方法：
// - GetCost: 获取总代价。
// - SetTrajCost, SetHeuCost: 设置轨迹代价和启发式代价。
// - GetPose: 获取节点的位置信息（x, y, phi）。
