...
bool ReadParkingScenario(ParkingScenario& parking_scenario) {
  std::ifstream file;
  std::string scenario_path = "/Users/michaelzhou/project/os/HAStar_ParkingPlanner/config/parking_scenario.txt";
  file.open(scenario_path, std::ios_base::in);
  if (!file.is_open()) {
    std::cerr << "Error: Could not open parking scenario file with absolute path: " << scenario_path << std::endl;
    return false;
  }
...
bool ReadHybridAStarParam(WarmStartConfig& warm_start_config) {
  std::ifstream file;
  std::string hya_param_path = "/Users/michaelzhou/project/os/HAStar_ParkingPlanner/config/hya_param.conf";
  file.open(hya_param_path, std::ios_base::in);
  if (!file.is_open()) {
    std::cerr << "Error: Could not open hybrid A* param file with absolute path: " << hya_param_path << std::endl;
    return false;
  }
...
bool ReadVehicleParam(VehicleParam& vehicle_param) {
  std::ifstream file;
  std::string vehicle_param_path = "/Users/michaelzhou/project/os/HAStar_ParkingPlanner/config/vehicle_param.conf";
  file.open(vehicle_param_path, std::ios_base::in);
  if (!file.is_open()) {
    std::cerr << "Error: Could not open vehicle param file with absolute path: " << vehicle_param_path << std::endl;
    return false;
  }
